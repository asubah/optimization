...
def method_0():
    ...
    for instance in instances:
        ...
        result.write('%d ' % original_cost.item())
...
def method_3():
    ...
    instances = os.listdir(path)
    # instances = ['bayg29.txt']
    # instances.sort()

    for instance in instances:
        result = open('Instances/' + instance + '.csv', 'w')
        result.write('I NEAREST FINAL TOUR\n')
        ...
        for i in range(0, m.shape[0]):
            print("========", "NEAREST NEIGHBOUR", "========")
            initial_tour, initial_cost = tm.ts_nearest_neighbour_single(i, m)
            result.write('%i %d ' % (i, initial_cost.item()))
            ...
            while True:
                ...
                if best_cost < initial_cost:
                    ...
                    result.write('%d ' % best_tour[0][0][0])
                    for edge in best_tour:
                        result.write('%d ' % edge[0][1])
